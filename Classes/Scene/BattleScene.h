...
private:
    ...
    // 音频系统
    int _combatPlanningMusicID = -1;  // 准备战斗音乐ID
    int _combatMusicID = -1;          // 战斗音乐ID
    ...
    cocos2d::Vec2 _currentTouchPos;       // 当前触摸位置（用于长按放置）
    bool _isTouchMoving;
    bool _isLongPressDeploying = false;   // 是否正在长按放置
    ...
    bool tryDeployTroopAt(const cocos2d::Vec2& touchPos);  // 尝试在指定位置放置兵种
    void stopLongPressDeploy();                             // 停止长按放置
    ...
    // 回放录制系统（委托给BattleRecorder）
    BattleRecorder _recorder;                // 回放录制/播放管理器
    bool _isInitialized = false;             // 是否完成初始化
