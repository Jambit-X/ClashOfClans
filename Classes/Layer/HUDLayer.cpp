...
// 自定义颜色
const Color3B COLOR_CYAN = Color3B(0, 255, 255);
const Color3B COLOR_ORANGE = Color3B(255, 165, 0);
...
bool HUDLayer::init() {
  ...
  if (goldIcon) {
    ...
    goldIcon->setAnchorPoint(Vec2(1, 0.5f));
  ...
  _goldLabel->enableShadow(Color4B(0, 0, 0, 150), Size(2, -2));
  ...
  _gemLabel->setColor(Color3B(0, 255, 0));
  ...
  // 调试按钮
  auto debugBtnBg = LayerColor::create(Color4B(255, 69, 0, 230), 70, 35);
  ...
  auto gradient = LayerGradient::create(
    Color4B(255, 100, 0, 200),
    Color4B(200, 30, 0, 200)
  );
  ...
  debugLabel->setPosition(Vec2(35, 17.5));
  debugLabel->setColor(Color3B::WHITE);
  debugLabel->enableOutline(Color4B(0, 0, 0, 255), 2);
  debugLabel->enableShadow(Color4B(0, 0, 0, 180), Size(2, -2));
...
void HUDLayer::updateResourceDisplay(int gold, int elixir) {
  ...
  if (_gemLabel) {
    _gemLabel->setString(StringUtils::format("%d", gem));
  }
}
...
void HUDLayer::initActionMenu() {
  ...
  _btnUpgrade->addClickEventListener([this](Ref*) {
    ...
    // 其他建筑的大本营等级限制
      ...
      if (!requirements->canUpgrade(building->type, building->level, currentTHLevel)) {
        ...
        showTips(reason, Color3B::ORANGE);
        return;
      }
    }
  ...
  // 研究按钮（实验室专用）
  _btnResearch = Button::create("UI/laboratory/research.png");
...
void HUDLayer::updateActionButtons(int buildingId) {
  ...
  _btnResearch->setPosition(layout->trainPos);
...
void HUDLayer::enterContinuousBuildMode(int buildingType) {
  ...
  this->addChild(_modeHintLabel, 999);
...
void HUDLayer::updateWorkerDisplay() {
  ...
  // 根据工人状态动态改变颜色
  if (idle == 0) {
    _workerLabel->setColor(Color3B::RED);
  } else if (idle < total) {
    _workerLabel->setColor(Color3B::ORANGE);
  } else {
    _workerLabel->setColor(COLOR_CYAN);
  }
}
